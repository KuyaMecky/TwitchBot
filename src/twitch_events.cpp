...
//                      Internal Functions
//#######################################################################
char* get_o_auth_token()
{
	return twitchState->oAuthToken;
}

bool refresh_o_auth_token()
{
	...
	{
		assert(0, "Failed to recieve HTTP Response: %s", refreshToken.url);
		return 0;
	}
	...
	{
		assert(0, "Failed to recieve HTTP Data: %s", refreshToken.url);
		return 0;
	}
...
void create_channel_point_redemption()
{
	char header[512] = {};
	...
	{
		assert(0, "Failed to recieve HTTP Response: %s", createChannelPointRewards.url);
	}
	char resultBuffer [1024] = {};
	if (!platform_receive_http_data(createChannelPointRewards, resultBuffer, 1024))
	{
		assert(0, "Failed to recieve HTTP Data: %s", createChannelPointRewards.url);
	}
...
bool init_twitch_connection(AppMemory* appMemory)
{
...
void handle_tts_request(char* redemptionBuffer, uint32_t redemptionTextLength,
																 bool chipmunk = false)
{
	...
	{
		TTSRequest request = {};
		memcpy(request.text, redemptionBuffer, redemptionTextLength);
		request.text[redemptionTextLength] = 0;
	...
	for (uint32_t requestIdx = 0; requestIdx < requestCount; requestIdx++)
	{
		...
		// Get MP3 File containing Brian Voice
		{
			...
			{
				assert(0, "Failed to receive HTTP Response: %s", getMP3FromStreamelements.url);
			}
			...
			{
				assert(0, "Failed to receive HTTP Data: %s", getMP3FromStreamelements.url);
			}

			platform_close_http_request(getMP3FromStreamelements);
		}

		assert(mp3DataSize < MP3_BUFFER_SIZE,
	...
	while (true)
	{
		...
		// Get Channel Point Redemptions
		{
					...
					redemptionTypeID++)
			{
				...
				{
					assert(0, "Failed to recieve HTTP Response: %s", getChannelPointRewards.url);
				}
				...
				{
					assert(0, "Failed to recieve HTTP Data: %s", getChannelPointRewards.url);
				}
				...
				switch(redemption.type)
				{
					...
					case REDEMPTION_TYPE_VIDEO:
					{
						if(twitchState->requestVideoIdx + 1 < MAX_REQUEST_VIDEOS)
						{
							...
							// Complete the Request using Twtich API
							{
								...
								{
									assert(0, "Failed to recieve HTTP Response: %s", completeRedemption.url);
								}
								...
								{
									assert(0, "Failed to recieve HTTP Data: %s", completeRedemption.url);
								}
					...
					default:
					assert(0, "Unknown Redemption Type: %d", redemption.type);
				}
			}
		}
...
void get_broadcaster_id()
{
	...
	{
		assert(0, "Failed to recieve HTTP Response: %s", getBroadcasterID.url);
		return -1;
	}
	memset(buffer, 0, 1024);
	if (!platform_recieve_http_data(getBroadcasterID, buffer, 1024))
	{
		assert(0, "Failed to recieve HTTP Data: %s", getBroadcasterID.url);
		return -1;
	}
